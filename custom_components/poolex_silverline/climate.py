...
from .entity import SilverlineEntity, compute_hvac_action
...
class SilverlineClimate(SilverlineEntity, ClimateEntity, RestoreEntity):
    ...
    def hvac_action(self) -> HVACAction | None:
        ...
        (DP 108 is absent on the minimal Poolex variant). The actual
        decision tree lives in entity.compute_hvac_action so non-climate
        consumers (the runtime_today accumulator) can call it directly.
        """
        state = self.coordinator.data
        if state is None:
            return None
